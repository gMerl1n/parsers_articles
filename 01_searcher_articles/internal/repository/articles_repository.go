...
func (a *ArticleRepo) CreateArticle(ctx context.Context, article entities.Article) (bool, error) {
	...
	query := fmt.Sprintf("INSERT INTO %s (author, title, body, url, provider_sign, published_at) VALUES ($1, $2, $3, $4, $5)", tableArticles)

	if err := a.db.QueryRow(ctx, query, article.Author, article.Title, article.Body, article.URL, article.ProviderSign, article.PublishedAt).Scan(); err != nil {
